...
> Updated for **OpenClaw 2026.5.22 stable** and the **2026.5.24-beta.1** policy/meeting-notes/voice/image changes. See [Part 26 — Migration Guide](../part26-migration-guide.md) if you're on an older version.
...
- **Local-only setup on a 14B-or-smaller model:** prefer `agents.list[].experimental.localModelLean: true` for that worker only; use provider-level `localService` only if OpenClaw should own the model daemon lifecycle.
...
- **Shared channels:** set `messages.queue.mode` intentionally, use `tools.toolsBySender`, and run `openclaw policy check` before exposing runtime/filesystem tools to public users.
