#!/usr/bin/env node
/**
 * update-mocs.mjs — MOC Health Check
 *
 * Finds broken wiki-links, stale completed items, and orphaned notes.
 *
...
function main() {
  if (!fs.existsSync(THINKING)) {
    console.log('No MOCs found (vault/01_thinking/ does not exist).');
    return;
  }
  ...
  for (const moc of mocs) {
    ...
    if (broken.length) {
      console.log(`${moc}`);
      for (const b of broken) console.log(`   BROKEN: [[${b}]] -> not found`);
    }
  }
