#!/usr/bin/env node
/**
 * process-inbox.mjs — Reviews vault/00_inbox/ and suggests filing locations.
 *
 * Usage:
 *   node process-inbox.mjs          # report only
 *   node process-inbox.mjs --auto   # auto-move files to suggested locations
 */
...
function main() {
  ...
  if (!fs.existsSync(INBOX)) {
    console.log('Inbox empty (directory does not exist).');
    return;
  }
  ...
  if (!autoMove && files.length) console.log(`\nRun with --auto to move files automatically.`);
}
